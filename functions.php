<?php
/**
 * Mominul functions and definitions
 *
...
if ( ! function_exists( 'mpt_option' ) ) {
	function mpt_option( $option = '', $default = null ) {
 ...
 */
function mominul_cat_count_span( $links ) {
...
add_filter( 'wp_list_categories', 'mominul_cat_count_span' );

/**
 * Filter the archives widget to add a span around post count
 */
function mominul_archive_count_span( $links ) {
	$links = str_replace( '</a>&nbsp;(', '<span class="post-count">', $links );
	$links = str_replace( ')', '</span></a>', $links );
...
add_filter( 'get_archives_link', 'mominul_archive_count_span' );

add_filter( 'get_archives_link', 'mominul_archive_count_span' );

if ( ! function_exists( 'mominul_reorder_comment_fields' ) ) {
	function mominul_reorder_comment_fields( $fields ) {
		$new_fields = array();
...
add_filter( 'comment_form_fields', 'mominul_reorder_comment_fields' );
...
add_action('mpt_cursor', 'mpt_cursor');
